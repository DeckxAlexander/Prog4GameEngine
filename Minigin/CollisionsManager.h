...
namespace dae
{
	...
	{
	public:
		const std::vector<CollisionComponent*>& GetCollisions() const { return m_Collisions; }
		void AddCollision(CollisionComponent* coll) { m_Collisions.push_back(coll); }
		void RemoveCollision(CollisionComponent* coll) { m_Collisions.erase(std::remove(m_Collisions.begin(), m_Collisions.end(), coll), m_Collisions.end()); }
	private:
